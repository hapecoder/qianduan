...
var add = (function () {
    var counter = 0;
    var a =function (){
        return counter += 1;   //访问到外部的counter变量
    }
    return a
})();
...
+ 引用类型：堆：Object，Array（数组 [] ,{}）、function、Date     运行时动态分配内存的，因此存取速度较慢。
...
关于JS中的一些类型确定，尽量使用Object.prototype.toString.call(obj) 去判定类型，使用Typeof会有一些缺陷。
